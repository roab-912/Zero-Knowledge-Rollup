...
Ce dossier fournit de quoi exécuter `scripts/bench/measure_zk_resources.py` sur un
...
docker compose -f scripts\rapidsnark\docker-compose.yml exec bench `
    python3 scripts/bench/measure_zk_resources.py --sizes 1,2,4,8 --skip-setup --repeat 5 `
...
docker compose -f scripts\rapidsnark\docker-compose.yml exec bench `
    python3 scripts/bench/measure_zk_resources.py --sizes 1,2,4 --work-dir /bench/circuits `
...
`scripts/bench/generate_proofs.py` et par `--rapidsnark-mode docker` — circuits montés
...
```bash
bash scripts/bench/rapidsnark/setup.sh
```
