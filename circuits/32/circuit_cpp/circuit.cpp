...
void VerifyNTransactions_14_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void VerifyNTransactions_14_run(uint ctx_index,Circom_CalcWit* ctx);
...
uint get_main_input_signal_no() {return 224;}

uint get_total_signal_no() {return 6398;}

uint get_number_of_components() {return 607;}

uint get_size_of_input_hashmap() {return 256;}

uint get_size_of_witness() {return 5088;}
...
RecursiveAnd_13_create(mySignalStart+257,0+ctx_index+1,ctx,new_cmp_name,myId);
mySubcomponents[32] = 0+ctx_index+1;
}
{
PFrElement aux_dest = &lvar[1];
// load src
...
Fr_lt(&expaux[0],&lvar[1],&circuitConstants[9]); // line circom 86
while(Fr_isTrue(&expaux[0])){
{
...
// load src
// end load src
Fr_copy(aux_dest,&signalValues[mySignalStart + ((1 * Fr_toInt(&lvar[1])) + 97)]);
}
// run sub component if needed
if(!(ctx->componentMemory[mySubcomponents[cmp_index_ref]].inputCounter -= 1)){
VerifyTransaction_8_run(mySubcomponents[cmp_index_ref],ctx);
...
// load src
// end load src
Fr_copy(aux_dest,&signalValues[mySignalStart + ((1 * Fr_toInt(&lvar[1])) + 161)]);
}
// run sub component if needed
if(!(ctx->componentMemory[mySubcomponents[cmp_index_ref]].inputCounter -= 1)){
VerifyTransaction_8_run(mySubcomponents[cmp_index_ref],ctx);
...
{
PFrElement aux_dest = &signalValues[mySignalStart + ((1 * Fr_toInt(&lvar[1])) + 225)];
// load src
...
Fr_lt(&expaux[0],&lvar[1],&circuitConstants[9]); // line circom 101
while(Fr_isTrue(&expaux[0])){
{
...
// load src
// end load src
Fr_copy(aux_dest,&signalValues[mySignalStart + ((1 * Fr_toInt(&lvar[1])) + 225)]);
}
// run sub component if needed
if(!(ctx->componentMemory[mySubcomponents[cmp_index_ref]].inputCounter -= 1)){
...
VerifyNTransactions_14_create(1,0,ctx,"main",0);
VerifyNTransactions_14_run(0,ctx);
}
