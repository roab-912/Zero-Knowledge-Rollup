...
void VerifyNTransactions_13_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void VerifyNTransactions_13_run(uint ctx_index,Circom_CalcWit* ctx);
...
uint get_main_input_signal_no() {return 112;}

uint get_total_signal_no() {return 3182;}

uint get_number_of_components() {return 303;}

uint get_size_of_input_hashmap() {return 256;}

uint get_size_of_witness() {return 2544;}
...
ctx->componentMemory[coffset].signalStart = soffset;
ctx->componentMemory[coffset].inputCounter = 112;
ctx->componentMemory[coffset].componentName = componentName;
ctx->componentMemory[coffset].idFather = componentFather;
...
RecursiveAnd_12_create(mySignalStart+129,0+ctx_index+1,ctx,new_cmp_name,myId);
mySubcomponents[16] = 0+ctx_index+1;
}
{
PFrElement aux_dest = &lvar[1];
// load src
...
Fr_lt(&expaux[0],&lvar[1],&circuitConstants[8]); // line circom 86
while(Fr_isTrue(&expaux[0])){
{
...
// load src
// end load src
Fr_copy(aux_dest,&signalValues[mySignalStart + ((1 * Fr_toInt(&lvar[1])) + 49)]);
}
// run sub component if needed
if(!(ctx->componentMemory[mySubcomponents[cmp_index_ref]].inputCounter -= 1)){
VerifyTransaction_8_run(mySubcomponents[cmp_index_ref],ctx);
...
// load src
// end load src
Fr_copy(aux_dest,&signalValues[mySignalStart + ((1 * Fr_toInt(&lvar[1])) + 81)]);
}
// run sub component if needed
if(!(ctx->componentMemory[mySubcomponents[cmp_index_ref]].inputCounter -= 1)){
VerifyTransaction_8_run(mySubcomponents[cmp_index_ref],ctx);
...
{
PFrElement aux_dest = &signalValues[mySignalStart + ((1 * Fr_toInt(&lvar[1])) + 113)];
// load src
...
Fr_lt(&expaux[0],&lvar[1],&circuitConstants[8]); // line circom 101
while(Fr_isTrue(&expaux[0])){
{
...
// load src
// end load src
Fr_copy(aux_dest,&signalValues[mySignalStart + ((1 * Fr_toInt(&lvar[1])) + 113)]);
}
// run sub component if needed
if(!(ctx->componentMemory[mySubcomponents[cmp_index_ref]].inputCounter -= 1)){
...
VerifyNTransactions_13_create(1,0,ctx,"main",0);
VerifyNTransactions_13_run(0,ctx);
}
