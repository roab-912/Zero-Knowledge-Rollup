...
void VerifyNTransactions_12_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void VerifyNTransactions_12_run(uint ctx_index,Circom_CalcWit* ctx);
...
uint get_total_signal_no() {return 1582;}

uint get_number_of_components() {return 151;}

uint get_size_of_input_hashmap() {return 256;}

uint get_size_of_witness() {return 1272;}
...
uint csoffset = mySignalStart+117;
...
RecursiveAnd_11_create(mySignalStart+65,0+ctx_index+1,ctx,new_cmp_name,myId);
mySubcomponents[8] = 0+ctx_index+1;
}
{
PFrElement aux_dest = &lvar[1];
...
// load src
// end load src
Fr_copy(aux_dest,&signalValues[mySignalStart + ((1 * Fr_toInt(&lvar[1])) + 41)]);
}
// run sub component if needed
if(!(ctx->componentMemory[mySubcomponents[cmp_index_ref]].inputCounter -= 1)){
VerifyTransaction_8_run(mySubcomponents[cmp_index_ref],ctx);
...
{
PFrElement aux_dest = &signalValues[mySignalStart + ((1 * Fr_toInt(&lvar[1])) + 57)];
// load src
...
Fr_lt(&expaux[0],&lvar[1],&circuitConstants[7]); // line circom 101
while(Fr_isTrue(&expaux[0])){
{
uint cmp_index_ref = 8;
{
...
// load src
// end load src
Fr_copy(aux_dest,&signalValues[mySignalStart + ((1 * Fr_toInt(&lvar[1])) + 57)]);
}
// run sub component if needed
if(!(ctx->componentMemory[mySubcomponents[cmp_index_ref]].inputCounter -= 1)){
...
VerifyNTransactions_12_create(1,0,ctx,"main",0);
VerifyNTransactions_12_run(0,ctx);
}
