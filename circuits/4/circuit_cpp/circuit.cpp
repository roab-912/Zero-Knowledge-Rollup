...
void VerifyNTransactions_11_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void VerifyNTransactions_11_run(uint ctx_index,Circom_CalcWit* ctx);
...
uint get_main_input_signal_no() {return 28;}

uint get_total_signal_no() {return 786;}
...
RecursiveAnd_10_create(mySignalStart+33,0+ctx_index+1,ctx,new_cmp_name,myId);
...
Fr_lt(&expaux[0],&lvar[1],&circuitConstants[6]); // line circom 86
while(Fr_isTrue(&expaux[0])){
{
...
// load src
// end load src
Fr_copy(aux_dest,&signalValues[mySignalStart + ((1 * Fr_toInt(&lvar[1])) + 21)]);
}
// run sub component if needed
if(!(ctx->componentMemory[mySubcomponents[cmp_index_ref]].inputCounter -= 1)){
VerifyTransaction_8_run(mySubcomponents[cmp_index_ref],ctx);
...
Fr_lt(&expaux[0],&lvar[1],&circuitConstants[6]); // line circom 101
while(Fr_isTrue(&expaux[0])){
{
uint cmp_index_ref = 4;
{
...
VerifyNTransactions_11_create(1,0,ctx,"main",0);
VerifyNTransactions_11_run(0,ctx);
}
