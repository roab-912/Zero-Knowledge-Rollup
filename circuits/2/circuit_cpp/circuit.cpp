...
void VerifyNTransactions_10_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void VerifyNTransactions_10_run(uint ctx_index,Circom_CalcWit* ctx);
...
uint get_total_signal_no() {return 390;}

uint get_number_of_components() {return 37;}

uint get_size_of_input_hashmap() {return 256;}

uint get_size_of_witness() {return 318;}
...
RecursiveAnd_9_create(mySignalStart+17,0+ctx_index+1,ctx,new_cmp_name,myId);
...
Fr_lt(&expaux[0],&lvar[1],&circuitConstants[5]); // line circom 86
while(Fr_isTrue(&expaux[0])){
{
...
Fr_lt(&expaux[0],&lvar[1],&circuitConstants[5]); // line circom 101
while(Fr_isTrue(&expaux[0])){
{
uint cmp_index_ref = 2;
{
...
VerifyNTransactions_10_create(1,0,ctx,"main",0);
VerifyNTransactions_10_run(0,ctx);
}
